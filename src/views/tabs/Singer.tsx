...
function useFixed(singers: Ref<Singers>, singersRef: Ref) {
  ...
  watch(scrollY, (val) => {
    ...
    if(index >= 1) {
      ...
      const diff = unref(distanceToTop) > 0 && unref(distanceToTop) < 30 ? unref(distanceToTop) - 30 : 0
  ...
  setup() {
    ...
    return () => (
      <div class={style.singer} v-loading={unref(loading)}>
        <Scroll ref={scrollRef} class={style['index-list']} options={{probeType: 3}} onScroll={onScroll}>
          {{
            default: () => (
              <>
                <ul ref={singersRef}>
                ...
                </ul>
                {
                   unref(fixedTitle) === ''
                   ? null
                   : <div class={style.fixed} style={{ transform: `translateY(${unref(fixedTranslateY)}px)` }}>
                      {unref(fixedTitle)}
                    </div>
                }
              </>
          ...
          <ul>
            {unref(singers).map((item, index) => (
              <li
                ...
                data-index={index}
                class={[style['shortcut-item'], unref(fixedIndex) === index? style['shortcut-item-active'] : ''].join(' ')}
              >
                  {item.title}
              </li>
            ))}
